...
class App {
    ...
    _findPositionForNewNoteInZone(zoneId) {
        const parentZone = this.state.getZones().find(z => z.id === zoneId);
        if (!parentZone) return this._getNewItemDesktopPosition(false); // Fallback si la zona no existe

        const notesInZone = this.state.getNotes().filter(n => n.zoneId === zoneId);
        const numCols = 4, numRows = 2;
        const grid = Array(numRows).fill(null).map(() => Array(numCols).fill(false));

        const gridX = parentZone.x + 15, gridY = parentZone.y + 45;
        const gridW = parentZone.width - 30, gridH = parentZone.height - 60;
        ...
        notesInZone.forEach(note => {
            const noteCenterX = note.x + (note.width / 2);
            const noteCenterY = note.y + (note.height / 2);
            const col = Math.floor((noteCenterX - gridX) / cellWidth);
            const row = Math.floor((noteCenterY - gridY) / cellHeight);
            if (row >= 0 && row < numRows && col >= 0 && col < numCols) {
                grid[row][col] = true;
            }
        });
        ...
        if (targetRow !== -1) {
            // Centrar la nota en la celda vacía encontrada
            const noteWidth = CONSTANTS.DEFAULT_NOTE_WIDTH;
            const noteHeight = CONSTANTS.DEFAULT_NOTE_HEIGHT;
            const cellCenterX = gridX + (targetCol * cellWidth) + (cellWidth / 2);
            const cellCenterY = gridY + (targetRow * cellHeight) + (cellHeight / 2);
            ...
            // Fallback: si la cuadrícula está llena, apilar debajo de la zona
            return { x: parentZone.x, y: parentZone.y + parentZone.height + 20 };
        }
    }
